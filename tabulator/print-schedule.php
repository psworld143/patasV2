...
// Fetch deductions for all contestants
$deductions = [];
$deduction_query = "SELECT * FROM deduction WHERE admin_id = ?";
$stmt_deductions = $con->prepare($deduction_query);
$stmt_deductions->bind_param("i", $admin_id);
$stmt_deductions->execute();
$result_deductions = $stmt_deductions->get_result();
while ($row = $result_deductions->fetch_assoc()) {
    $deductions[$row['contestant_id']][] = $row;
}
...
.custom-button img {
   ...
   style="margin-top: 20px; margin-left: 10px; background-color: rgb(240,158,65); width: 150px;">
   <img src="../asset/img/print.png" width="30">
   Print
</a>
                  </div>
...
<div class="modal fade" id="showDeductionModal" tabindex="-1" aria-labelledby="showDeductionModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="showDeductionModalLabel">Deductions</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div id="deductionsContent">
                    <!-- Deductions content will be dynamically filled by JavaScript -->
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $('.show-deductions-btn').on('click', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#deductionsContent').html(response);
                $('#showDeductionModal').modal('show');
            },
            error: function(xhr, status, error) {
                console.error("AJAX Error: " + status + " - " + error);
                alert('Error fetching deductions');
            }
        });
    });
});
